...
contract EduChain {
    ...
    event AchievementAdded(string indexed studentId, string courseName, uint256 grade);

    // Модификатор для проверки прав вуза
    modifier onlyUniversity() {
        require(universities[msg.sender], "Only authorized universities can add data");
        _;
    }
    ...
    function getAchievementsByStudent(string memory _studentId) external view returns (Achievement[] memory) {
        uint256 count = 0;
        for (uint256 i = 0; i < achievements.length; i++) {
            if (keccak256(bytes(achievements[i].studentId)) == keccak256(bytes(_studentId))) {
                count++;
            }
        }
